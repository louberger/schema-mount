...
+ Run-time: the mounted schema is defined by instance data
...
module.  Note however that any YANG version, including version 1, modules can be mounted under
a mount point.
...
* Augment, Validation and Parent References in Mounted Data
...
model as specified in ^I-D.ietf-rtgwg-device-model^ Section 2, using both logical
...
+ Requiring use of schema mount module for inline type mount points.
    From contributors discussion, Lada:
    ...
    I would propose to move the "schema-mount" extension to a separate
    module and then use, as you suggested, the "inline" case for mount
    points that do not have an entry in the "mount-point" list.

    But then we probably don't need the explicit "inline" case under
    "mount-point" at all: "mount-point" entries that are present would
    use the "use-schema" case and those that are missing use the
    "inline" case. This is already a nice simplification.

+ Recursion: does it require any specific support in the module or is
  inclusion of the schema mount module as a mounted module supported
  like any other module?

+ Filtering/limiting parent references, e.g., vrf-blue can only
  references interfaces assigned to vrf-blue
